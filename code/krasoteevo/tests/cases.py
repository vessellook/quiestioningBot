...
def case_sentence_1():
    ...
    json_str = open(f'{_dir_path}/data/sentence_1_json').read()
    return sentence, json.loads(json_str)


def case_sentence_2():
    sentence = 'Маша и Петя любили зверей, и мама отвела их в зоопарк.'
    json_str = open(f'{_dir_path}/data/sentence_2_json').read()
    return sentence, json.loads(json_str)


def case_sentence_3():
    sentence = 'Ваня идёт гулять.'
    json_str = open(f'{_dir_path}/data/sentence_3_json').read()
    return sentence, json.loads(json_str)
